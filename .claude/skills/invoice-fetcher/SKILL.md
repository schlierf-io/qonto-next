...
- **Vendor routing** — `VENDOR_ROUTES` in [lib/gmail/query.ts](lib/gmail/query.ts)
  ...
  - **Claude.ai / ANTHROPIC / "Claude (Anthropic)"** → `from:(anthropic.com OR
    stripe.com) "Anthropic"` (PBC/USD via `mail.anthropic.com`, the EUR Claude
  ...
  - **Google Workspace/Cloud/One/Wallet** → `from:(google.com) "Google"` (invoices
    ...
    go to a different mailbox and won't be found here.)
